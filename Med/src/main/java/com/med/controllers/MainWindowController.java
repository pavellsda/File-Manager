...
public class MainWindowController {
    ...
    public ListView listView; //Список стандартных каталогов(Home, Documents...)
    final ContextMenu contextMenu = new ContextMenu(); //Меню, вызываемое по правому щелчку мыши
    public ListView listFind; //Список, выводящий результаты поиска
    ...
    private void initListFind(){
         ...
         */
        listFind.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                listFind.setVisible(false);
                tableView.setVisible(true);
                String path = listFind.getSelectionModel().getSelectedItem().toString();
    ...
    private void initListView(){
        ...
        listView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                switch(listView.getSelectionModel().getSelectedItem().toString()){
                    case "Home":
                        setCurPath(path);
                        history.add(path);
                        label_current_folder.setText(getCurPath());
    ...
    private void initContextMenu(){
         ...
         */
        cut.setOnAction(ev -> menuCut(ev));
        copy.setOnAction(ev -> menuCopy(ev));
        paste.setOnAction(ev -> menuPaste(ev));
        delete.setOnAction(ev -> menuDelete(ev));
    }
    ...
    private void initDirTree(){
        ...
        tree_folders_list.setCellFactory((p) -> {
    ...
    private void initFileTable() {
        ...
        TableColumn<Path, Path> column1 = new TableColumn<Path, Path>("File name");
        ...
        TableColumn<Path, String> column3 = new TableColumn<Path, String>("Date");
        ...
        TableColumn<Path, String> column2 = new TableColumn<Path, String>("File size");
        column2.setCellValueFactory((TableColumn.CellDataFeatures<Path, String> arg0) -> {
           return new StringBinding() {
                @Override
                protected String computeValue() {
    ...
    public void menuCut(ActionEvent ev){
     ...
     */
    public void menuCopy(ActionEvent ev){
     ...
     */
    public void menuDelete(ActionEvent ev){
     ...
     */
    public void menuPaste(ActionEvent ev) {
    ...
    public void addDir(){
        ...
        if (result.isPresent()){
            ...
            File newDir = new File(getCurPath()+folderName);
            newDir.mkdirs();
        }
        updatePath();
    }
            ...
            for (File file2 : list) {
                if (file2.isDirectory()) {
                    ...
                    Platform.runLater(new Runnable() {
                        @Override
                        public void run() {
                            listFind.getItems().add(file2.getParentFile().toString() + "`" + file2.getName());
                        }
                    });

                    //setCurPath(file2.getParentFile().toString());
                    //history.add(getCurPath());
                }
            }
        }
    }
     ...
     */
    public void copyDir(File src, File dest, boolean cut)
            throws IOException{
